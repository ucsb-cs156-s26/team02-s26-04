...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three UCSBOrganizations correctly for regular user", async () => {
    ...
    expect(
      ...
      ucsbOrganizationFixtures.threeUcsbOrganizations[0].orgCode,
    );

    expect(
      screen.getByTestId(`${testId}-cell-row-1-col-orgCode`),
    ).toHaveTextContent(
      ucsbOrganizationFixtures.threeUcsbOrganizations[1].orgCode,
    );

    expect(
      screen.getByTestId(`${testId}-cell-row-2-col-orgCode`),
    ).toHaveTextContent(
      ucsbOrganizationFixtures.threeUcsbOrganizations[2].orgCode,
    );

    expect(
      screen.getByTestId(`${testId}-cell-row-0-col-orgTranslationShort`),
    ).toHaveTextContent(
      ucsbOrganizationFixtures.threeUcsbOrganizations[0].orgTranslationShort,
    );

    expect(
      screen.getByTestId(`${testId}-cell-row-0-col-orgTranslation`),
    ).toHaveTextContent(
      ucsbOrganizationFixtures.threeUcsbOrganizations[0].orgTranslation,
    );

    expect(
      screen.getByTestId(`${testId}-cell-row-0-col-inactive`),
    ).toHaveTextContent(
      `${ucsbOrganizationFixtures.threeUcsbOrganizations[0].inactive}`,
    );
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSBOrganization with orgCode ACG was deleted");
